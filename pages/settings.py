...
from lib.ouraAPI import oura_connected
from lib.stravaApi import strava_connected
from lib.withingsAPI import withings_connected
from lib.sqlalchemy_declarative import db_connect, stravaSummary, ouraSleepSummary, athlete, hrvWorkoutStepLog
from lib.datapull import refresh_database
...
import configparser
import operator
from lib.fitlyAPI import hrv_training_workflow
from dash_app import dash_app

config = configparser.ConfigParser()
config.read('./config.ini')

layout = html.Div(id='settings-canvas', children=[
...
@dash_app.callback([Output('ftp-test-notification-threshold-input-submit', 'style'),
...
def set_ftp_threshold(n_clicks, value):
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.ftp_test_notification_week_threshold:
            ...
            engine.dispose()
            session.close()
            dash_app.server.logger.info('Updated ftp week threshold to {}'.format(value))
            if success:
...
@dash_app.callback([Output('daily-sleep-goal-input-submit', 'style'),
...
def sleep_goal_status(n_clicks, value):
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.daily_sleep_hr_target:
            ...
            engine.dispose()
            session.close()
            dash_app.server.logger.info('Updated daily sleep hour goal to {}'.format(value))
            if success:
                return {'display': 'none'}, {'color': 'green', 'paddingLeft': '1vw', 'fontSize': '150%'}
            else:
                return {'paddingLeft': '1vw', 'paddingRight': '0vw', 'display': 'inline-block', 'border': '0px'}, {
...
@dash_app.callback([Output('weekly-tss-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.weekly_tss_goal:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated weekly TSS goal to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
            engine.dispose()
...
@dash_app.callback([Output('rr-max-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.rr_max_goal:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated Max Ramp Rate Injury treshold to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
            engine.dispose()
...
@dash_app.callback([Output('rr-min-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.rr_min_goal:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated Min Ramp Rate Injury treshold to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
            engine.dispose()
...
@dash_app.callback([Output('min-workout-time-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.min_non_warmup_workout_time:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated min time to consider an activity a workout to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
            engine.dispose()
...
@dash_app.callback([Output('weekly-workout-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.weekly_workout_goal:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated weekly workout goal to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
            engine.dispose()
...
@dash_app.callback([Output('weekly-yoga-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.weekly_yoga_goal:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated weekly yoga goal to {}'.format(value))
                dash_app.server.logger.info('Updated weekly yoga goal to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
...
@dash_app.callback([Output('weekly-sleep-score-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.weekly_sleep_score_goal:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated weekly sleep score goal to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
            engine.dispose()
...
@dash_app.callback([Output('weekly-readiness-score-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.weekly_readiness_score_goal:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated weekly readiness score goal to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
            engine.dispose()
...
@dash_app.callback([Output('weekly-activity-score-goal-input-submit', 'style'),
    ...
    if n_clicks and n_clicks > 0:
        ...
        if value == athlete_info.weekly_activity_score_goal:
            ...
            try:
                ...
                session.commit()
                success = True
                dash_app.server.logger.info('Updated weekly activity score goal to {}'.format(value))
            except BaseException as e:
                success = False
                dash_app.server.logger.error(e)
            engine.dispose()
...
def set_fitness_goals(readiness_dummy, hrv_dummy, readiness_switch, hrv_switch):
    ...
    try:
        if athlete_info.weekly_yoga_goal != weekly_yoga_goal and athlete_info.weekly_workout_goal != weekly_workout_goal:
            dash_app.server.logger.info('Updating weekly yoga goal to {}'.format(
                weekly_yoga_goal if weekly_yoga_goal != 99 or weekly_yoga_goal != 100 else 'readiness score based'))
            dash_app.server.logger.info('Updating weekly workout goal to {}'.format(
...
@dash_app.callback(Output('api-connections', 'children'),
                   [Input('url', 'pathname')])
...
# Manual Refresh
@dash_app.callback(Output('refresh-status', 'children'),
                   [Input('refresh-db-button', 'n_clicks')])
def refresh(n_clicks):
    if n_clicks > 0:
        dash_app.server.logger.info('Manually refreshing database tables...')
...
@dash_app.callback(Output('truncate-hrv-status', 'children'),
                   [Input('truncate-hrv-button', 'n_clicks')],
                   [State('truncate-date', 'date')])
def reset_hrv_plan(n_clicks, hrv_date):
    if n_clicks > 0:
        dash_app.server.logger.info('Resetting HRV workout plan workflow to step 0 on {}'.format(hrv_date))
        try:
            session, engine = db_connect()
            ...
            session.rollback()
            dash_app.server.logger.error('Error resetting hrv workout plan: {}'.format(e))
...
# Truncate database
@dash_app.callback(Output('truncate-refresh-status', 'children'),
...
def truncate_and_refresh(n_clicks, n_clicks_date, truncateDate):
    if n_clicks > 0:
        dash_app.server.logger.info('Manually truncating and refreshing database tables...')
        try:
...
@dash_app.callback(Output('logs', 'children'),
...
# Clear Logs button
@dash_app.callback(Output('clear-log-dummy', 'children'),
                   [Input('clear-log-button', 'n_clicks')])
def clear_logs(n_clicks):
    if n_clicks and n_clicks > 0:
        open('./log.log', 'w').close()
        dash_app.server.logger.debug('Logs manually cleared')
    return 'Logs last cleared at {}'.format(datetime.utcnow())


# Set log level
@dash_app.callback([Output('info-log-button', 'style'),
...
def set_log_level(info_n_clicks, error_n_clicks, debug_n_clicks, info_n_clicks_timestamp,
    ...
    if info_n_clicks_timestamp != 0 or error_n_clicks_timestamp != 0 or debug_n_clicks_timestamp != 0:
        ...
        dash_app.server.logger.setLevel('INFO')
        dash_app.server.logger.info('Log level set to {}'.format(latest))
        dash_app.server.logger.setLevel(latest)
